...
const gameController = (function (){
    ...
    function playMove(row, column){
        if (gameOver){
            gameOverContent.innerText = "Game is already over."
            gameOverBox.appendChild(gameOverContent);
        ...
        if (board[row][column] !== undefined){
            gameOverContent.innerText = "Invalid move: Cell Already Occupied";
            gameOverBox.appendChild(gameOverContent);
        ...
        if (result !== null){
            gameOver = true;
            resultContent.innerText = `${activePlayer.name} wins!`;
        ...
        if (checkTie()){
            gameOver = true;
            resultContent.innerText = "It's a tie!";
            resultBox.appendChild(resultContent);
...
const displayController = function () {
    ...
    function resetBoard() {
        ...
        cells.forEach(cell => {
            cell.innerText = "";
            cell.classList.remove("cellDecoration");
        });
...
resetButton.addEventListener("click", () => {
    gameController.startGame(player1NameInput.value, player2NameInput.value);
    displayController.resetBoard();
});
